...
def get_team_with_reports(department: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for row in rows:
            d = dict(row)
            d['last_report_date'] = _ts(d.get('last_report_date'))
...
def get_user_reports_paginated(user_id: int, limit: int = 10, offset: int = 0) -> Tuple[List[Dict[str, Any]], int]:
    ...
    try:
        ...
        rows = cursor.fetchall()
        return [
            {'id': r[0], 'text': r[1], 'timestamp': _ts(r[2]),
...
def get_user_burnout_trend(user_id: int, days: int = 30) -> Dict[str, Any]:
    ...
    history = [{"date": _ts(row[0])[:10], "burnout": row[1]} for row in reversed(rows)]
    current = rows[0][1]
...
def get_company_burnout_history(days: int = 30) -> List[Dict[str, Any]]:
    ...
    return [{"date": _ts(row[0])[:10], "burnout": round(row[1], 4)} for row in reversed(rows)]
...
def get_departments_burnout_history(days: int = 30) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for row in rows:
        dept = row[0]
        date = _ts(row[1])[:10]
        result.setdefault(dept, []).append({"date": date, "burnout": round(row[2], 4)})
    for dept in result:
...
def get_department_reports_stats() -> List[Dict[str, Any]]:
    ...
    return [
        {"department": r[0], "report_count": r[1],
         "percentage": round((r[1] / total) * 100)}
        for r in rows
    ]
...
def get_company_burnout_stats() -> Dict[str, Any]:
    ...
    cursor = conn.cursor()
    try:
        # Средний burnout по последнему отчёту каждого сотрудника
        cursor.execute('''
    ...
    try:
        ...
        if department:
            q = base_q.format(
                ...
                dept_filter='u.department = %s AND'
            )
...
def get_user_score_trend(user_id: int, days: int = 7) -> Optional[Dict[str, Any]]:
    ...
    for row in rows:
        ts = row[0] if isinstance(row[0], datetime) else datetime.strptime(_ts(row[0])[:10], '%Y-%m-%d')
