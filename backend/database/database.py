"""
Слой доступа к данным (PostgreSQL через psycopg 3).
...
from psycopg_pool import ConnectionPool  # В psycopg3 пул вынесен в отдельный модуль
...
def get_db_connection():
    ...
    global pool
    if pool is None:
        # В psycopg3 пул создается иначе
        pool = ConnectionPool(
            conninfo=DATABASE_URL,
            min_size=DB_POOL_MIN_CONN,
            max_size=DB_POOL_MAX_CONN,
            open=True  # Автоматически открыть пул
        )
    # В psycopg3 пул сам управляет соединениями, не нужно getconn/putconn
    return pool.connection()


def release_db_connection(conn):
    """Освобождение соединения (в psycopg3 не требуется, но оставим для совместимости)"""
    # В psycopg3 соединение автоматически возвращается в пул при закрытии
...
def init_db():
    ...
    with get_db_connection() as conn:  # Используем контекстный менеджер
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            try:
                ...
                conn.commit()
                return user_id
            except psycopg.errors.UniqueViolation:  # В psycopg3 ошибки в psycopg.errors
                conn.rollback()
                return None
...
def get_all_users() -> List[Dict[str, Any]]:
    with get_db_connection() as conn:
        # Используем row_factory для получения словарей
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            try:
                ...
                # В psycopg3 конфликт обрабатывается через ON CONFLICT
                cursor.execute(
...
def _fetch_reports_with_analysis(cursor, where_clause: str, params: tuple) -> List[Dict[str, Any]]:
    ...
    result = []
    for row in rows:
        # В psycopg3 row может быть кортежем или словарем в зависимости от row_factory
...
def get_user_reports(user_id: int) -> List[Dict[str, Any]]:
    with get_db_connection() as conn:
        with conn.cursor() as cursor:  # Без row_factory для совместимости с _fetch_reports_with_analysis
...
# Остальные функции (calculate_weighted_score, get_user_weighted_score и т.д.)
# остаются без изменений, так как они работают со структурой данных,
# а не напрямую с БД
